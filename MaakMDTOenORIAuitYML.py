#!/usr/bin/env python3
"""Maak ORI-A en samenhangende MDTO XML uit Notubiz-YAML.
...
Alle benoemde agenda-items worden als zelfstandige agendapunten opgenomen,
ook wanneer een agenda-item in de bron een bovenliggend agenda-item heeft.
Voor de vergadering en ieder agendapunt wordt een MDTO-informatieobject
geschreven. Agendapunten verwijzen met ``isOnderdeelVan`` naar de vergadering.
...
en bestand-sidecar volgens MDTO-XML 1.0.1. Documenten verwijzen met
``isOnderdeelVan`` naar hun agendapunt; meetingmedia naar de vergadering.

Voorbeeld:

    python3 MaakMDTOenORIAuitYML.py \
        --bronmap input \
        --uitvoermap output \
...
@dataclass(frozen=True)
class Documentoverdracht:
    """Document met de direct bovenliggende MDTO-aggregatie."""
...
def zichtbare_agenda_bomen(items: Any) -> list[dict[str, Any]]:
    """Maak alle benoemde agenda-items zelfstandig, ongeacht hun parent."""
    ...
    for item in items:
        ...
        if zichtbaar_agenda_item(item):
            kopie = dict(item)
            # In deze variant worden kinderen niet genest in het ORI-A-object.
            kopie["_zichtbare_kinderen"] = []
            resultaat.append(kopie)
        resultaat.extend(zichtbare_agenda_bomen(item.get("agenda_items")))
    return resultaat
...
def bouw_mdto_informatieobject(
    ...
    if onderdeel_van_id is not None and onderdeel_van_naam is not None:
        onderdeel_van = mdto_element(informatieobject, "isOnderdeelVan")
        mdto_verwijzing(
            onderdeel_van,
            onderdeel_van_naam,
            onderdeel_van_id,
...
def iter_agenda_bomen(items: list[dict[str, Any]]):
    """Doorloop de zelfstandig gemaakte zichtbare agenda-items."""
    for item in items:
        yield item
...
def mdto_agendapunt_naam(agenda_item: dict[str, Any]) -> str:
    """Maak de betekenisvolle naam die in MDTO en verwijzingen wordt gebruikt."""
    prefix, _ = agenda_item_kop(agenda_item)
    prefixtekst = str(prefix).replace(".", "").strip() if heeft_waarde(prefix) else ""
    naam = agenda_item_naam(agenda_item)
    return f"Agendapunt {prefixtekst}: {naam}" if prefixtekst else f"Agendapunt: {naam}"
...
def bouw_agendapunt(
    ...
    start_date: Any,
    media: list[dict[str, Any]],
    documenten_voor_overdracht: list[Documentoverdracht],
) -> None:
    ...
    if isinstance(documenten, list):
        for document in documenten:
            if not isinstance(document, dict) or not is_bestandsdocument(document):
                continue
            documenten_voor_overdracht.append(
                Documentoverdracht(
                    document=document,
                    onderdeel_van_id=maak_id("A", agenda_id),
                    onderdeel_van_naam=mdto_agendapunt_naam(agenda_item),
                )
            )
...
def bouw_ori_xml(
    ...
    ET.ElementTree,
    list[Documentoverdracht],
    list[dict[str, Any]],
]:
    """Bouw ORI-A met ieder benoemd agenda-item als zelfstandig agendapunt."""
    ...
    documenten_voor_overdracht: list[Documentoverdracht] = [
        Documentoverdracht(
            document=mediabron,
            onderdeel_van_id=maak_id("V", meeting_id),
            onderdeel_van_naam=f"{meetingnaam} {datum}",
        )
        for mediabron in media
    ]
    ...
    return ET.ElementTree(root), documenten_voor_overdracht, agenda_bomen
...
def schrijf_agendapunten_mdto_xml(
    ...
    meeting_id, meetingnaam, datum = meeting_mdto_gegevens(meeting)
    vergadering_id = maak_id("V", meeting_id)
    vergadering_naam = f"{meetingnaam} {datum}"
    for agenda_item in agenda_items:
        agenda_id = normaliseer_id(agenda_item.get("id"))
        if agenda_id is None:
            print(
                "Waarschuwing: agenda-item zonder id heeft geen MDTO-bestand gekregen",
                file=sys.stderr,
            )
            continue
        objectnaam = nummering.kandidaat()
        doelbestand = meetingmap / f"{objectnaam}.mdto.xml"
        tijdelijk_bestand = meetingmap / f".{objectnaam}.mdto.xml.tmp"
        try:
            boom = bouw_mdto_informatieobject(
                maak_id("A", agenda_id),
                mdto_agendapunt_naam(agenda_item),
                datum,
                "Agendapunt",
                "Dossier",
                vergadering_id,
            ...
            schrijf_mdto_xml(boom, tijdelijk_bestand)
            tijdelijk_bestand.replace(doelbestand)
        except (OSError, TypeError, ValueError) as fout:
            tijdelijk_bestand.unlink(missing_ok=True)
            print(
                f"Waarschuwing: MDTO voor agenda-item {agenda_id} kon niet "
...
def verwerk_ymls(
    ...
    for bronbestand, meeting in meetings:
        ...
        try:
            boom, documenten, agenda_items = bouw_ori_xml(meeting)
...
def parse_parameters() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Lees Notubiz-YAML, genereer ORI-A zonder subagendapunten en "
            "samenhangende MDTO XML voor vergaderingen, agendapunten en "
            "documenten/media."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Genereer ORI-A en MDTO voor vergadering en agendapunten zonder "
            "documenten of media over te dragen"
        ),
    )
    parser.add_argument(
        "--overschrijf",
